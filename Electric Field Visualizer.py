...
cross-sections. You can adjust the plane of the cross-section by changing the angles α (alpha) and β (beta),
as well as the height. The script also provides the option to display electric field lines on the 2D cross-section.

Features:
- Compute electric potential and field for different cathode configurations.
- Visualize cross-sectional slices of the electric field magnitude.
- Interactive sliders and buttons to adjust viewing parameters.
- Option to display electric field lines on the 2D plot.
...
- Use the sliders and buttons to adjust viewing parameters.
- Toggle the display of electric field lines.
...
def open_video_settings(event):
    ...
    def capture_current_cross_section():
        # Create a new figure
        cross_section_fig, cross_section_ax = plt.subplots()
        # Copy the image data
        ...
        cross_section_ax.set_title(ax_2d.get_title())
        cross_section_ax.set_xlabel(ax_2d.get_xlabel())
        cross_section_ax.set_ylabel(ax_2d.get_ylabel())

        # Plot field lines if present
        ...
        if file_path:
            # Save the cross-section figure
            cross_section_fig.savefig(file_path, dpi=300, bbox_inches='tight')
